...
scl, scb = int(screen_size.width), int(screen_size.height)
...
def rotate_point(pos, img, angle):
    ...
    newx = x * cos(radians(angle)) + y * sin(
        radians(angle)) + img.shape[1] * 0.4
    newy = -x * sin(radians(angle)) + y * cos(
        radians(angle)) + img.shape[0] * 0.4
...
while running:
    ...
    for event in pygame.event.get():
        ...
        and here is where you should do it.

        I'm just too lazy to.

        '''

        # we don't care, we just need this to be called
        # in order for pygame to function properly
        print(event)
